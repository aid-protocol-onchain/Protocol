...
export function DonatePanel({ campaignId }: { campaignId: string }) {
  ...
  const controls = (
    <>
      <div className="field-label">{t("donate.currency")}</div>
      <div className="seg">
        {ASSETS.map((a) => (
      ...
      <div className="field-label">{t("donate.amount", { unit })}</div>
      ...
      <div className="toggle">
        <span><i className="ti ti-eye-off" style={{ verticalAlign: -2 }} aria-hidden="true" /> {t("donate.giveAnonymously")}</span>
        <input type="checkbox" checked={anon} onChange={(e) => setAnon(e.target.checked)} />
      </div>
      <button className="btn btn-primary btn-block" onClick={donate} disabled={busy}>
        <i className="ti ti-heart" aria-hidden="true" />{" "}
        {state === "approving" ? t("donate.approveInWallet") : busy ? t("donate.confirmInWallet") : t("donate.donateAmount", { amount, unit })}
      </button>
      {state === "done" && (
        <div className="note" style={{ color: "var(--trust)" }}>
          <i className="ti ti-circle-check" aria-hidden="true" /> {t("donate.recorded")}{" "}
          <a href={txUrl} target="_blank" rel="noreferrer">{t("donate.viewTransaction")}</a>
        </div>
      )}
      {state === "err" && (
  ...
  return (
    <div className="panel donate">
      <h2 style={{ fontSize: 17, margin: "0 0 4px" }}>{t("donate.title")}</h2>

      {evmChains.length === 0 && !solCampaign ? (
        <div className="note" style={{ marginTop: 10 }}>
          <i className="ti ti-info-circle" aria-hidden="true" /> {t("donate.notDeployed")}
        </div>
      ) : showConnectPrompt ? (
        <div className="note" style={{ marginTop: 10 }}>
          <i className="ti ti-wallet" aria-hidden="true" /> {t("donate.connectPrompt", { accepts })}
        </div>
      ) : onSolana && !solCampaign ? (
        <div className="note" style={{ marginTop: 10 }}>
          <i className="ti ti-info-circle" aria-hidden="true" /> {t("donate.notOnSolana", { accepts })}
        </div>
      ) : wrongChain ? (
        <>
          <div className="note" style={{ marginTop: 10 }}>
            <i className="ti ti-switch-horizontal" aria-hidden="true" /> {t("donate.wrongChain", { network: CHAIN_NAME[activeChainId] ?? "an unsupported network", accepts })}
          </div>
          {evmChains.length > 0 && (
            <button className="btn btn-primary btn-block" onClick={() => switchChain({ chainId: target })}>
              <i className="ti ti-switch-horizontal" aria-hidden="true" /> {t("donate.switchTo", { network: CHAIN_NAME[target] })}
            </button>
          )}
        </>
      ) : ready ? (
        <>
          <div className="faint" style={{ margin: "6px 0 2px" }}>
            <i className="ti ti-link" style={{ verticalAlign: -2 }} aria-hidden="true" /> {t("donate.donatingOn", { network: onSolana ? "Solana devnet" : CHAIN_NAME[activeChainId] })}
          </div>
          {controls}
        </>
      ) : null}
      ...
      <div className="note">
        <i className="ti ti-lock" aria-hidden="true" /> {t("donate.footer")}
      </div>
    </div>
  );
}
